#if !defined HARDROCK50_H_DEFINED
#define HARDROCK50_H_DEFINED
...
class CHardrock50 : public CHardrock {
public:
  CHardrock50(CSerialDevice& rDevice)
    : CHardrock(rDevice),
...
private:
  CHardrock50();
  CHardrock50(const CHardrock50&);
  CHardrock50& operator=(const CHardrock50&);
...
public:
  ...
  virtual bool isTunerByPassed(void) {
    ...
    if (availableForWrite()) {
      ...
      if (isValidResponse(Rsp, Cmd)
          && Rsp.length() > 5
          && (Rsp.charAt(5) == '0' || Rsp.charAt(5) == '1')) {
        return Rsp.charAt(5) == '0';
      }
    }
    return false;
  }
